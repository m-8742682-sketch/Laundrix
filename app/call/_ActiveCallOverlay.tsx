/**
 * ActiveCallOverlay - Shows when an active call is minimized
 * 
 * INTEGRATED VERSION - Uses centralized call state
 */
...
import { container } from "@/di/container"; // ✅ ADDED
...
export default function ActiveCallOverlay() {
  ...
  const hasAddedCallRecord = useRef(false); // ✅ ADDED

  // Subscribe to active call data and screen state
  useEffect(() => {
  ...
  const addCallRecordToChat = async (status: "ended" | "missed", duration: number) => {
    ...
    try {
      ...
      console.log(`[ActiveCallOverlay] Added call record: ${status}, ${duration}s`);
    } catch (error) {
  ...
  const endCall = async () => {
    ...
    // ✅ ADDED: Add call record before clearing state
