...
export const processScanResult = (response: ScanResponse): {
  ...
  // Always check result field first — backend returns success:false for unauthorized
  // (HTTP 403) but still includes result:"unauthorized" + data.incidentId which we need.
  if (response.result && stateMap[response.result]) {
    return {
      state: stateMap[response.result],
      data: response.data,
      message: response.message,
    };
  }

  // Only fall back to "error" if there's truly no usable result
  return {
    state: "error",
